...
- `src/test/java/de/sharpsharp/gildedrose/`: JUnit 4, Hamcrest, Mockito, ApprovalTests. 25 Tests aus der Original-Kata, je Warenart eine Klasse (`AGildedRoseItem`, `AnAgedBrie`, `ABackstagePass`, `TheHandOfRagnaros`), Namen und Grenzen in `TestConstants`; sie erreichen jede Zeile und jeden Zweig von `GildedRose`. Surefire führt jede Klasse unter `src/test` aus, der Name muss nicht auf `Test` enden. Unter `approval/` die zwei Approval-Tests der Original-Kata mit genehmigten Dateien, im Original mit `@Ignore`; das `@Ignore` dort ist Vorgabe, nicht Verstoß.
